...
const Advance = () => {
  ...
  const [loadError, setLoadError] = useState({ Loan: null, Advance: null });
  const MONTHS = [
    "January",
    "February",
    "March",
  ...
  // Each tab deliberately reads only its own production table.
  const loadData = useCallback(async () => {
    const module = activeTab;
    setLoading(true);
    setLoadError(current => ({ ...current, [module]: null }));
    try {
      if (module === "Advance") {
        const result = await fetchAdvancesPaginated({ page, pageSize });
        setAdvances(result.data);
        setTotalRecords(result.totalRecords);
        setTotalPages(result.totalPages || 1);
      } else {
        const result = await fetchSalaryAdvances();
        setSalaryAdvances(result);
      }

      if (isAdmin) {
        const [employeeData, configData] = await Promise.all([
          fetchEmployees().catch(err => {
            console.warn("[Advance] fetchEmployees warning:", err);
            return [];
          }),
          fetchSalaryConfigs().catch(err => {
            console.warn("[Advance] fetchSalaryConfigs warning:", err);
            return [];
          })
        ]);
        setEmployeesList((employeeData || []).filter(e => e.status === "active" || !e.status).map(e => ({
      ...
      console.error(`[${module}] loadData error:`, err);
      setLoadError(current => ({ ...current, [module]: err }));
    }
    setLoading(false);
  }, [activeTab, isAdmin, page]);
    ...
    reader.onload = async e => {
      try {
        ...
        const rows = raw
          ...
          .map(r => {
            const employeeName = idx.employeeName >= 0 ? String(r[idx.employeeName] || "").trim() : "";
            const matchedEmployee = employeesList.find(employee =>
        ...
        toast.success(`Imported ${insertedRows.length} loan record(s) successfully.`);
      } catch (err) {
  ...
  const handleImportSalaryAdvanceFile = file => {
    ...
    reader.onload = async e => {
      try {
        ...
        const firstCellRaw = String(raw[0][0] || "")
          .trim()
          .toLowerCase();
        ...
        if (isSalarySummaryFormat) {
          ...
          if (skipped > 0) {
            toast.success(
              `Imported ${validRows.length} advance record(s). Skipped ${skipped} — employee name not matched.`
            );
          } else {
        ...
        if (skipped > 0) {
          toast.success(
            `Imported ${validRows.length} advance record(s). Skipped ${skipped} record(s) — employee not matched or amount exceeded salary.`
          );
        } else {
          toast.success(`Imported ${validRows.length} advance record(s) successfully.`);
        }
  ...
  // Filter advances/loans based on role in the frontend
  const displayedAdvances = isAdmin
    ? advances
    : advances.filter(loan => loan.employee_id === employeeId);
            ...
            <div className="p-4 md:p-6">
                      ...
                      <tr>
                        <td colSpan={isAdmin ? 8 : 6} className="px-6 py-12 text-center text-gray-500">
                          <div className="flex justify-center flex-col items-center">
                            ...
                            <span className="text-sm font-medium">Loading loan requests...</span>
                          </div>
                        </td>
                      </tr>
                    ) : loadError.Loan ? (
                      <tr>
                        <td colSpan={isAdmin ? 9 : 7} className="px-6 py-8 text-center text-red-600">
                      ...
                      displayedSalaryAdvances.map(adv => (
                        <tr key={adv.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                            ₹
                            {parseAmount(adv.amount || adv.total_amount).toLocaleString("en-IN", {
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ₹
                            {parseAmount(adv.monthly_deduction || adv.monthlyDeduction).toLocaleString("en-IN", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2
                            })}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                            ...
                            {parseAmount(
                              adv.remaining_amount !== undefined && adv.remaining_amount !== null
                                ...
                                : adv.amount || adv.total_amount
                            ).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {adv.deduction || "Yes"}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{adv.reason}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium flex items-center w-fit ${adv.status === "Approved"
                                ? "bg-green-100 text-green-800"
                                : adv.status === "Fully Paid"
                            ...
                            >
                              ...
                              {(adv.status === "Approved" || adv.status === "Fully Paid") && (
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t border-gray-100 bg-white gap-3 rounded-b-2xl">
                <div className="text-xs text-gray-500 font-medium">
                  Showing{" "}
                  <span className="font-bold text-gray-800">{totalRecords > 0 ? (page - 1) * pageSize + 1 : 0}</span> to{" "}
              ...
              <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                  <h2 className="text-lg font-semibold text-gray-900">
                    {editingLoanId ? "Edit Loan" : "Request Loan"}
                  </h2>
                  <button
                    onClick={() => {
                      setShowModal(false);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            {isAdmin && (
              <div className="px-4 py-2 bg-red-50 border border-red-200 rounded-lg w-fit">
                ...
                <span className="text-sm font-bold text-red-800 ml-1">₹{liveTotalDebit.toLocaleString("en-IN")}</span>
              </div>
            )}
          ...
          {isAdmin && unmatchedImportNames.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 md:p-6">
              ...
              <p className="text-xs text-yellow-700 mb-3">
                These names from the uploaded sheet didn't match any active employee. Fix the spelling in the sheet, or
                add/activate the employee, then re-import.
              </p>
              <div className="flex flex-wrap gap-2">
                {unmatchedImportNames.map((name, i) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-white border border-yellow-300 rounded-full text-xs font-medium text-yellow-800"
                  >
                    {name}
                  </span>
                ))}
                      ...
                      displayedAdvances.map(adv => (
                        <tr key={adv.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            >
                              {adv.status === "Pending" && <Clock size={12} className="mr-1" />}
                              {(adv.status === "Approved" || adv.status === "Deducted") && (
                                <Check size={12} className="mr-1" />
                              )}
                              {adv.status === "Rejected" && <X size={12} className="mr-1" />}
                              {adv.status}
          ...
          {showAdvanceModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                  <h2 className="text-lg font-semibold text-gray-900">
                    {editingSalaryAdvanceId ? "Edit Advance" : "Request Advance"}
                  </h2>
                  <button
                    onClick={() => {
                      setShowAdvanceModal(false);
          ...
          <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-8">
              ...
              <div className="mb-8">
                ...
                <div className="overflow-x-auto bg-slate-50 rounded-2xl border border-slate-200 shadow-inner p-1">
                  <table className="min-w-full border-collapse">
                    <thead>
                      <tr>
                        {(activeTab === "Loan" ? SALARY_ADVANCE_FIELDS : ADVANCE_FIELDS).map(field => (
                          <th
                            key={field.key}
                            className="px-6 py-4 text-left border-r border-slate-200 last:border-0 min-w-[140px]"
                          >
                    ...
                    <tbody>
                      <tr className="bg-white">
                        {(activeTab === "Loan" ? SALARY_ADVANCE_FIELDS : ADVANCE_FIELDS).map(field => (
                          <td
                            key={field.key}
                            className="px-6 py-5 border-r border-slate-200 last:border-0 bg-slate-50/30"
                          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  onClick={() => downloadCsvTemplate(activeTab === "Loan" ? SALARY_ADVANCE_FIELDS : ADVANCE_FIELDS)}
